...
def convert_machine_axis_to_world(results,width,height,x_mid,y_mid,depth,name):
    ...
    for i in [1.05,0.85,0.95,1.15,1.25]:
        if 0<alpha<=0.25*pi or 1.75 *pi<alpha<=2*pi:
            alpha=0
            theta1=0
            i-=0.2
        if (alpha > 0.25 * pi and alpha < 0.75 * pi):
            i+=0.2
            alpha=0.5*pi
            theta1=pi/2
        if (alpha >= 1.25 * pi and alpha < 1.75 * pi):
            i+=0.2
            alpha=1.5*pi
            theta1=-pi/2
        if alpha > 0.75 * pi and alpha <= 1.25 * pi:
            alpha=pi
            theta1=pi
            i+=0.3
        ...
        if (alpha > 0.25 * pi and alpha < 0.75 * pi) or (alpha >= 1.25 * pi and alpha < 1.75 * pi):
            z_second+=0.2
        ...
        rospy.loginfo("this is x1  %f and this is y1 %f and name is %s",x1,y1,name)
        machine_x,machine_y=get_map_pose(machine_odom_now_x,machine_odom_now_y)
        rospy.loginfo("this is  odom x1  %f and this is odom y1 %f and name is %s",machine_x,machine_y,name)
        # theta1 = alpha + rpy[2]
        rospy.loginfo('角度: %f', alpha)
        reach_place2.append((x1,y1,theta1))
    
    
    
    d=depth[x_mid][y_mid]
    real_x,_,real_z=real_pose(x_mid,y_mid,d)
    ...
    map_x=x_person
    map_y=y_person
    reach_place2_copy=[(map_x-0.85,map_y,0),(map_x,map_y+0.85,-math.pi/2),(map_x,map_y-0.85,math.pi/2),(map_x+0.85,map_y,math.pi),
                  (map_x-1,map_y,0),(map_x,map_y+1,-math.pi/2),(map_x,map_y-1,math.pi/2),(map_x+1,map_y,math.pi),
                  (map_x-1.15,map_y,0),(map_x,map_y+1.15,-math.pi/2),(map_x,map_y-1.15,math.pi/2),(map_x+1.15,map_y,math.pi)]
    distance_list=[]
    for (x,y,theta) in reach_place2_copy:
    ...
    for i in range(len(distance_list)):
        for j in range(i,len(distance_list),1):
            if distance_list[i]>distance_list[j]:
                reach_place2_copy[i],reach_place2_copy[j]=reach_place2_copy[j],reach_place2_copy[i]  
    reach_place2.extend(reach_place2_copy)
...
def mediapipe_task(image,mutex,name,width,height,mid_x,mid_y,depth):
    ...
    if results.pose_world_landmarks!=None:
        ...
        # rospy.loginfo("%s",human_result)
            ...
            map_x=x_person
            map_y=y_person
            reach_place2=[(map_x-0.85,map_y,0),(map_x,map_y+0.85,-math.pi/2),(map_x,map_y-0.85,math.pi/2),(map_x+0.85,map_y,math.pi),
                  (map_x-1,map_y,0),(map_x,map_y+1,-math.pi/2),(map_x,map_y-1,math.pi/2),(map_x+1,map_y,math.pi),
                  (map_x-1.15,map_y,0),(map_x,map_y+1.15,-math.pi/2),(map_x,map_y-1.15,math.pi/2),(map_x+1.15,map_y,math.pi)]
            distance_list=[]
