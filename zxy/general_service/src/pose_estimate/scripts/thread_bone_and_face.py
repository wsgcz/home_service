...
person_index=0
begin_recogine=None
def Draw_circle(posetion_arr:list,length:int,img,depth):
    d0=0
    d1=0
    for i in range(7,length,3):
        ...
        if i==25:
            d0=depth[pos_y][pos_x]
        if i==22:
            d1=depth[pos_y][pos_x]
    print("this is do ",d0,"this is d1",d1 )
# 读取摄像头画面
...
# 姿势识别
    ...
    for idx in range(output.shape[0]):
        ...
        Draw_circle(output[idx],len(output[idx]),nimg,depth)
