...
def control_angle_speed():
    ...
    if can_not_know_the_deepth:
        ...
        if now_time_postion_x>0.8:
            ...
            now_angle_speed=-now_time_postion_x*0.45
...
def reset_speed():
    ...
    rate.sleep()
def grab_the_object():
    global now_time_postion_y,have_set_everything,begin_back,over_yolo
...
def calculate_ahead_speed():
    global now_time_postion_z,now_ahead_speed,have_left
...
def calculate_postion_data(mutex):
    ...
    mutex.acquire()
    yolo_result_message, _ = yolo.inference(img=color_image, if_show=True)
    ...
    try:
        ...
        for person_i in person:
            ...
            # print(d)
                ...
                y=real_y
                fuy=-real_y
                real_z=z*math.cos(angle)+fuy*math.sin(angle)+0.07
                rospy.loginfo("this is real_z %f" ,real_z)
                real_y=-z*math.sin(angle)+fuy*math.cos(angle)+1.15
...
if __name__=="__main__":
    ...
    yolo_result_sub=rospy.Subscriber("yolo_result",String,yolo_result_cb,queue_size=10)
    # time.sleep(5)
    # mg=machince_grab()
