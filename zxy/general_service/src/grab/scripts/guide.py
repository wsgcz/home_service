...
class machince_grab:
    ...
    def do_pos(self):
        global number_image,now_have_been_middle_place,all_object_need,start_guide_time
        ...
        if data_in_yolo is None:
            ...
            if len(msg_data.keys())!=0:
                ...
                if self.label in list(msg_data.keys()):
                    ...
                    temp_judge_flag=1
                    self.pianzhi=0
                    for i in msg_data.keys():
                        if (i in all_object_need):
                            temp_judge_flag=0
                            the_thing_need=msg_data[i]
                            break
                    if temp_judge_flag:
                        self.pianzhi=0
                        the_thing_need=msg_data[list(msg_data.keys())[0]]
                ...
                if d>0.0000001:
                    if self.target_count==0:
                        ...
                        reach_place1=[(map_x,map_y+0.5,-math.pi/2),(map_x,map_y-0.5,math.pi/2),(map_x+0.5,map_y,math.pi),(map_x-0.5,map_y,0)]
                        reach_place2=[(map_x,map_y+1.1,-math.pi/2),(map_x,map_y-1.1,math.pi/2),(map_x+1.1,map_y,math.pi),(map_x-1.1,map_y,0)]
                        reach_temp=[]
                        self.judge_arrive_result=[]
                        no_can_go_flag=1
                        no_can_go_flag_index=0
                        for i in range(4):
                            temp_l=[reach_place1[i],reach_place2[i]]
                            reach_temp.append(temp_l)
                        # for i in range(4):
                        #     for j in range(i+1,4):
                        #         if (abs(reach_temp[i][0][0]-now_x)>abs(reach_temp[j][0][0]-now_x)):
                        #             reach_temp[i],reach_temp[j]=reach_temp[j],reach_temp[i]
                        for i in range(4):
                            for j in range(i+1,4,1):
                                
                                rospy.loginfo("%s",reach_temp[i][1])
                                if (abs(reach_temp[i][0][1]-all_room_y[self.grab_room_name])>abs(reach_temp[j][0][1]-all_room_y[self.grab_room_name])):
                                    reach_temp[i],reach_temp[j]=reach_temp[j],reach_temp[i]
                        for i in range(4):
                            reach_place1[i]=reach_temp[i][0]
                            reach_place2[i]=reach_temp[i][1]
                        rospy.loginfo("%s",reach_temp)
                        rospy.loginfo("reach_plsce1 %s",reach_place1)
                        rospy.loginfo("reach_plsce2 %s",reach_place2)
                        
                            
                        # for index,data in enumerate(reach_place2):
                        #     distance1=data[0]-all_room_corner[self.grab_room_name][0]
                        # dis=[]
                        (x1,y1,theta1)=(0,0,0)
                        if now_have_been_middle_place==1:
                            ...
                            while(True):
                                ...
                                for i in range(4):
                                    ...
                                    if self.judge_arrive_result[i] <=253 and self.judge_arrive_result[i+4] <253:
                                        ...
                                        break
                                    if self.judge_arrive_result[i+4]<253:
                                        no_can_go_flag_index=i
                                if no_can_go_flag:
                                    (x1,y1,theta1)=reach_place2[no_can_go_flag_index]                            
                                break
                            ...
                            jiaodu=math.atan(xiangdui_x/xiangdui_z)
                            ...
                            add_theta1 =  rpy[2]
                            rospy.loginfo("this is theta1 data is %f",theta1)
                            rospy.loginfo("this is  %f ",add_theta1)
                            # if self.grab_room_name=="bedroom":
                                # rospy.loginfo("this is self.machine thta %f",theta1)
                            theta1=jiaodu+add_theta1
                                # theta1*=(-1)
                ...
                rospy.loginfo("have passed 5 second %f",time.time()-start_guide_time)
                
                if time.time()-start_guide_time>6 and self.rotate_count==0:
                    start_guide_time=time.time()
                    rospy.loginfo("have entered 5 second insight")
                    vel_speed=0.25
                    nothing_found_flag=1
                    self.turn_right_or_left(vel_speed)
                    for i in range(3):
                        self.pub_loc.publish(self.twist_msg)
                    while (not rospy.is_shutdown()):
                        if nothing_found_flag==1:
                            if time.time()-start_guide_time>3:
                                # if nothing_found_flag==0:
                                    vel_speed*=-1
                            ...
                            if time.time()-start_guide_time>6:
                                    vel_speed*=-1
                                    # nothing_found_flag=0
                                    self.turn_right_or_left(vel_speed)
                                    for i in range(3):
                                    ...
                                    self.pub_loc.publish(self.twist_msg)
                                    start_guide_time=time.time()
                        if len(self.yolo_result.keys())!=0:
                            for i in range(10):
                                self.stop()
                                rospy.loginfo("break")
                                self.pub_loc.publish(self.twist_msg)
