...
void domsg(const std_msgs::StringConstPtr& msg_p) {
    ...
    if (data.find("否") != data.npos) {
        ...
        pub_sleep.publish(msg_sleep);
        ros::param::set(msg_res.data, layer);
    } 
    else {
        ...
        if (data.find("第三层") != data.npos) {
            layer = 3;
            slayer = " 在第三层";
        }
        else if (data.find("第二层") != data.npos) {
            layer = 2;
            slayer = " 在第二层";
        }
        else if (data.find("第一层") != data.npos) {
            layer = 1;
            slayer = " 在第一层";
        }
        if (msg_res.data == "stop") {
            ss<<" please check it";
        }
        else {
            ss<<slayer<<" please check it";
        }
        tmpmsg.data=ss.str();
...
{	
    ...
    pub_res=nh.advertise<std_msgs::String>("/ourlisten",10);
