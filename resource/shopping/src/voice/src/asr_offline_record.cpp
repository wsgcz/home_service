...
const char * ASR_RES_PATH        = "fo|/home/gcz/shopping/src/voice/bin/msc/res/asr/common.jet"; //离线语法识别资源路径
const char * GRM_BUILD_PATH      = "/home/gcz/shopping/src/voice/bin/msc/res/asr/GrmBuilld"; //构建离线语法识别网络生成数据保存路径
const char * GRM_FILE            = "/home/gcz/shopping/src/voice/bin/call.bnf"; //构建离线识别语法网络所用的语法文件
...
class ros_voice
{
	...
	ros_voice(ros::NodeHandle &nh)
	{
		...
		pub=nh.advertise<std_msgs::String>("/ourspeak",10);
		pub1=nh.advertise<std_msgs::String>("/getmsg",10);
		pub2=nh.advertise<std_msgs::String>("/start",10);
		sub=nh.subscribe("/start",10,&ros_voice::domsg,this);
		ROS_INFO("语音接收模块启动成功！");
		ros::spin();
	}
	void domsg(const std_msgs::String::ConstPtr& msg)
	{
		...
		{	
			...
			const char *login_config    = "appid = 8e6b9e2e"; //登录参数
			UserData asr_data; 
			int ret                     = 0 ;
