...
sys.path.append('/home/zkw/general_service/src/yolov5-in-ros/scripts')
...
def grab_the_object():
    ...
    rospy.sleep(2)
    control_arm_data.position[0]=final_position_y+0.2
    control_arm_data.position[1]=0
    control_arm_data.velocity[1]=0.5
    control_arm_data.velocity[0]=0.5
    arm_action_pub.publish(control_arm_data)
...
def say_word_task(yolo_message:dict,img):#将在draw_images上识别的每一个物体扣出来，放到/grab/image_data/文件中
    global draw_image
    ...
    for i in yolo_message.keys():
        ...
        path="/home/zkw/general_service/src/grab/image_data/"
        path=path+i+".jpg"
    ...
    path_image="/home/zkw/general_service/src/grab/image_data/all.jpg"
    cv2.imwrite(path_image,draw_image)
    
def calculate_postion_data(mutex):#存储识别的物体，存储到对应位置，更新一些数据
    ...
    if object_need_to_grab not in list(yolo_result_message.keys()):
        ...
        if (machine_odom_distance<have_left and begin_ahead_function==1):#有一种是在旋转寻找，一种是旋转确定方向后机器人前进的情况
...
def back_and_reset():
    ...
    machine_speed.linear.x=-0.1   
    speed_of_pub.publish(machine_speed)
    rospy.sleep(rospy.Duration(4.0))   #机器人向后推0.4米
    reset_speed()
