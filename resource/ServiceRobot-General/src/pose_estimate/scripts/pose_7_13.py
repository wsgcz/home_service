...
def task1():
    ...
    while (cap.isOpened()):
        ...
        image = image.half()
# 姿势识别
        cv2.imwrite("/home/linxi/ServiceRobot-General/src/pose_estimate/data_face/medi00.jpg",image)
        ...
        for idx in range(output.shape[0]):
            ...
            if (count%30==0):
                task_new=threading.Thread(target=mediapipe_detect,args=(change_image,))
                task_new.start()
                person_index+=1
            # count+=1
            # mm=threading.Lock()
...
def image_callback(image_rgb,image_depth):
    rospy.loginfo("111111111111")
    global flag,person_index,people_can_get_count,loc_quene,all_work_count
    if all_work_count<=1:
    ...
    # 姿势识别
        ...
        nimg = cv2.cvtColor(nimg, cv2.COLOR_RGB2BGR)
        # print(output.shape)
        cv2.imwrite("/home/linxi/ServiceRobot-General/src/pose_estimate/data_face/medi00.jpg",nimg)
        ...
        for idx in range(output.shape[0]):
            ...
            # Draw_circle(output[idx],len(output[idx]),nimg)
            change_image=nimg[y_center-half_h:y_center+half_h,x_center-half_w:x_center+half_w]
            # depth=depth[y_center-half_h:y_center+half_h,x_center-half_w:x_center+half_w]
    ...
    # 打开摄像头
        ...
        if (not loc_quene.empty()):
            # rospy.loginfo(l)
            loc_target,name,image_name=loc_quene.get()
            rospy.loginfo("this is target %s and size is %d",loc_target,loc_quene.qsize())
            ac.send_goal(loc_target)
            ac.wait_for_result()
            # rospy.loginfo(loc_quene.get())
            rospy.loginfo("this is loc_quene size %d",loc_quene.qsize())
    all_work_count+=1
...
def convert_machine_axis_to_world(results,width,height,x_mid,y_mid,depth):
    ...
    # y1=-y1
    rospy.loginfo("this is x1  %f and this is y1 %f",x1,y1)
...
def task2(mutex,name):
    ...
    #         continue
    #     # img = cv2.imdecode(np.fromfile(os.path.join("/home/linxi/ServiceRobot-General/src/pose_estimate/data_face",data), dtype=np.uint8), -1)
    #     rospy.loginfo(os.path.join("/home/linxi/ServiceRobot-General/src/pose_estimate/data_face",data))
    #     # result = face_recognitio.register(img, user_name=f'person{name}{person_index}')
...
def mediapipe_task(image,mutex,name,width,height,mid_x,mid_y,depth):
    ...
    if results.pose_world_landmarks!=None:
        ...
        final_rslt=convert_machine_axis_to_world(results,width,height,mid_x,mid_y,depth)
    else :
...
def mediapipe_detect(image:np.ndarray,width,height,mid_x,mid_y,depth,name):
    global mp_pose, mediapipe_detect_count,person_index,people_can_get_count
    # mutex_all.acquire()
    ...
    task_me=Mediapipe_detect(target=mediapipe_task,image=image,mutex=mutex1,path1=f"/home/linxi/ServiceRobot-General/src/pose_estimate/data_face/{name}{last_person}.jpg",width=width,height=height,mid_x=mid_x,mid_y=mid_y,depth=depth)
    store_path=f"/home/linxi/ServiceRobot-General/src/pose_estimate/data_face/{name}{last_person}.jpg"
    task_face=Face_detect(target=task2,mutex=mutex2,name=store_path)
    task_me.start()
    task_face.start()
    task_me.join()
    task_face.join()
