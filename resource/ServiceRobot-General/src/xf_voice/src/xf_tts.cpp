...
{
    int         ret                  = MSP_SUCCESS;
    const char* login_params         = "appid =69667434, work_dir = .";//登录参数,appid与msc库绑定,请勿随意改动
...
//   ros::NodeHandle nh;
//   ros::Publisher pub=nh.advertise<std_msgs::Stringgeneral_serivce_start
//   SAYIT;
...
{
    public:
    xf_speak(ros::NodeHandle n)
    {
        pub=n.advertise<std_msgs::String>("/general_serivce_start",10);
        sub =n.subscribe("/general_service_xfsaywords",1000,&xf_speak::xfcallback,this);
        the_ans_sub=n.subscribe("general_service_judge_the_correct",10,&xf_speak::xfjudge,this);
        // ROS_INFO("00000000000000000000000000000000");
        ros::spin();
    }
    void xfcallback(const std_msgs::String::ConstPtr& msg)
    {general_serivce_start
        {
            std_msgs::String data;
            data.data="sssss";
            ROS_INFO("5555555555555555555555555555555555555");
            pub.publish(data);
    ...
    {
    // if ((msg->data=="one")||(msg->data=="two")||(msg->data=="three"))
    // {
        
    //     // xf_tts("Hello, I'm Xi Jiao Robot, what's your name?","/Robot/voice/wav/say.wav");
    //     // sprintf(cmd,"echo %s>/Robot/cmd/saywords","Hello, I'm Xi Jiao Robot, what's your name?");
    //     // popen(cmd,"r");
    //     // SAYIT;
    ...
    if ((msg->data=="one")||(msg->data=="two")||(msg->data=="three"))
    {
        ROS_INFO("have entered once");
        // ros::Duration(1).sleep();
        xf_tts("what do you need","/Robot/voice/wav/say.wav");
        sprintf(cmd,"echo %s>/Robot/cmd/saywords","what do you need");
        ROS_INFO("_________________________________________________________");
        popen(cmd,"r");
        SAYIT;
        if (ros::ok())
        {
            // ROS_INFO("lllllllllllllllllll");
            std_msgs::String data;
            data.data="sssss";
            pub.publish(data);
        }
    }  
    else if (msg->data=="zero")
    {
        xf_tts("where do i need to go","/Robot/voice/wav/say.wav");
        sprintf(cmd,"echo %s>/Robot/cmd/saywords","where do i need to go");
        ROS_INFO("_________________________________________________________");
        popen(cmd,"r");
        SAYIT;
    ...
    {
        xf_tts("sorry ,i don't understand ,please say again.","/Robot/voice/wav/say.wav");
        sprintf(cmd,"echo %s>/Robot/cmd/saywords","sorry ,i don't understand ,please say again");
        popen(cmd,"r");
        SAYIT;
    ...
    void xfjudge(const std_msgs::String::ConstPtr& msg)
    {
        if(msg->data=="yes")
            judge=1;
    }
