...
int main(int argc, char *argv[]) {
  ...
  std::unordered_map<int, const char *> grab_count_to_names;
  std::vector<int> done_index;
  front_goals.clear();
  grab_count_to_names[0] = "Alice";
  grab_count_to_names[1] = "Bob";
  grab_count_to_names[2] = "Carol";
  ...
  ros::Subscriber register_subscriber{
      node_handle.subscribe("general_service_register_face", 10, RegisterCallBack)};
  ...
  while (ros::ok()) {
    if (now_state == AllStates::kWaitEnter) {
      ...
      ac.waitForServer();
      ac.sendGoal(front_goals[recognition_count]);
      ac.waitForResult();
      while (ac.getState() != actionlib::SimpleClientGoalState::SUCCEEDED) {
        ac.waitForServer();
        ac.sendGoal(front_goals[recognition_count]);
      ...
      ROS_INFO("到达第 %d 个目标", recognition_count);
      if (names_to_goal_index.count(grab_count_to_names[grab_count]) == 0) {
      ...
      if (std::find(done_index.begin(), done_index.end(), 2) ==
          done_index.end()) {
        recognition_count = 2;
      } else if (std::find(done_index.begin(), done_index.end(), 1) ==
                 done_index.end()) {
        recognition_count = 1;
      } else {
        recognition_count = 0;
      ...
      if (object_names.size() == current_person_index + 1 && face_register_flag) {
      ...
      if (recognition_message.compare("Alice") == 0) {
        ROS_INFO("找到第 %d 个人是 Alice", recognition_count);
        names_to_goal_index["Alice"] = recognition_count;
        if (std::strcmp(grab_count_to_names[grab_count], "Alice") != 0) {
          ++recognition_count;
          if (recognition_count > 2) {
            recognition_count = 0;
          }
          while (std::find(done_index.begin(), done_index.end(),
                           recognition_count) != done_index.end()) {
            ++recognition_count;
          }
          if (recognition_count > 2) {
            recognition_count = 0;
          ...
          done_index.emplace_back(recognition_count);
        }
        now_state = AllStates::kPut;
      } else if (recognition_message.compare("Bob") == 0) {
        ROS_INFO("找到第 %d 个人是 Bob", recognition_count);
        names_to_goal_index["Bob"] = recognition_count;
        if (std::strcmp(grab_count_to_names[grab_count], "Bob") != 0) {
          ++recognition_count;
          if (recognition_count > 2) {
            recognition_count = 0;
          }
        ...
        now_state = AllStates::kPut;
      } else if (recognition_message.compare("Carol") == 0) {
        ROS_INFO("找到第 %d 个人是 Carol", recognition_count);
        names_to_goal_index["Carol"] = recognition_count;
        if (std::strcmp(grab_count_to_names[grab_count], "Carol") != 0) {
          ++recognition_count;
          if (recognition_count > 2) {
            recognition_count = 0;
          }
