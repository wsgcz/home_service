...
#     Y = (v_img-p_y)*Z/f_y
#     y=abs(Y/1000)
#     zzz= d/1000*math.cos(0.53)+y*math.sin(0.53)
#     yyy=1.64-d/1000*math.sin(0.53)+y*math.cos(0.53)
...
def image_callback(image_rgb: Image, image_depth: Image):
    global image, depth, eps, goals, this_switch, rotate_flag, start_time, bridge, success_time,time_of_pass,last_time
    now_time=rospy.Time.now().to_sec()
    if (now_time-last_time>time_of_pass):
        ...
        for person in persons:
            ...
            if person[0] >= 474 and person[0] <= 486:
                ...
                #这部分是用来处理重影问题的
                the_real_x1=real_pose(x_mid-width/2,y_mid,average_depth)[0]
                the_real_x2=real_pose(x_mid+width/2,y_mid,average_depth)[0]
                the_real_width=the_real_x2-the_real_x1
                the_real_width=abs(the_real_width)
                print(the_real_width,average_depth)
                half_angle=math.atan(the_real_width/(2*(average_depth/1000)))
                total_angle=2*half_angle
                total_angle=total_angle/math.pi*180
                time_of_pass=total_angle/10+0.2
                last_time=rospy.Time.now().to_sec()            
    else :
        print("now the time is ",rospy.Time.now().to_sec())
        print("the pass of time is ",time_of_pass)
        return            
