...
def register(image, user_name, faces_embedding):  # 传入abc的名字以及图片
    ...
    if len(faces) != 1:  # 注册时只能读一张人脸
        cv2.imwrite('/home/linxi/ServiceRobot-General/src/general_service_2022/scripts/cache/' + str(false_count) + '.jpg', image)
        false_count += 1
...
def face_detection_call_back(msg: String):
    ...
    if face_detection_message == 'offoff':
        yolo_person_start_publisher.publish(String('off'))


def yolo_result_call_back(msg: String):
    global yolo_result_message
    yolo_result_message = msg.data


def image_call_back(msg: Image):
    global bridge, yolo_result_message, faces_embedding
    state = ''
    if face_detection_message == 'off' or face_detection_message == 'offoff':
        return
        ...
        if 'person' not in yolo_result_message:
            rotate(10)
            state = 'search'
        else:
            persons = []
            yolo_result_dict = json.loads(yolo_result_message)
...
if __name__ == '__main__':
    rospy.init_node(name='general_service_face', anonymous=True)
    face_detection_subscriber = rospy.Subscriber(
        name='general_service_face_detection', data_class=String, queue_size=1, callback=face_detection_call_back)
    yolo_result_subscriber = rospy.Subscriber(
        name='yolo_result', data_class=String, queue_size=1, callback=yolo_result_call_back)
    ...
    recognition_publisher = rospy.Publisher(
        'general_service_recognition', String, queue_size=1)
    vel_pub = rospy.Publisher('/cmd_vel', Twist, queue_size=1)
    # pool=threadpool.ThreadPool(4)
    yolo_person_start_publisher = rospy.Publisher(
        'general_service_yolo_person_start', String, queue_size=10
    )
    faces_embedding = list()
    rospy.spin()
