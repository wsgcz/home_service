...
class Yolov7:
    def __init__(self, model_path='/home/cch/general_service/src/pose_estimate/scripts/yolov7-w6-pose.pt'):
        # 初始化Yolov7类，model_path参数指定模型权重文件的路径
        ...
        # 将模型转换为半精度（FP16）并移动到指定设备（GPU或CPU）
        self.model = self.model.half().to(self.device)

        # 将模型设置为评估模式，这会影响某些层的行为，如BatchNorm和Dropout
        self.model.eval()
    ...
    # 处理选中的检测结果
        for idx in idx_list:
            ...
            # 在原图上绘制矩形框
            cv2.rectangle(nimg, 
                      (int(output[idx][2]-output[idx][4]/2), int(output[idx][3]-output[idx][5]/2)),
                      (int(output[idx][4]/2+output[idx][2]), int(output[idx][5]/2+output[idx][3])),
