...
class StartTrain:
    def __init__(self,data="", epochs=10, imgsz=640, device="cuda", workers=0, batch=4, cache=True):
        self.data = data
        self.epochs = epochs
        self.imgsz = imgsz
        self.device = device
...
class StartVal:
    def __init__(self,data="", imgsz=640, batch=4, conf=0.25, iou=0.6, device='cuda', workers=0):
        self.data = data
        self.imgsz = imgsz
        self.batch = batch
        self.conf = conf
...
class StartPredict:
    ...
    def start_single_predict(self, yolo_model, img_path):
        model = YOLO(yolo_model).to(self.device)
...
def main():
    ...
    start_train = StartTrain(data=data, epochs=epochs, imgsz=imgsz, batch=batch, device=device)
    start_val = StartVal(data=data, imgsz=imgsz, batch=batch, device=device)
