...
sys.path.insert(0,'/home/zkw/general_service/src/pose_estimate')
sys.path.insert(1,'/home/zkw/general_service/src/pose_estimate/scripts')
...
weigths = torch.load('/home/zkw/general_service/src/pose_estimate/scripts/yolov7-w6-pose.pt')
model = weigths['model']
...
def task3(mutex,name):
    ...
    begin=time.time()
    # names_list =os.listdir("/home/zkw/general_service/src/pose_estimate/data_face")
    # names_list.reverse()
...
def add_name_to_list():
    ...
    face_db_store_name=os.listdir("/home/zkw/general_service/src/pose_estimate/face_db")
...
def image_callback(image_rgb,image_depth):
    ...
    # 姿势识别
        ...
        # print(output.shape)
        cv2.imwrite("/home/zkw/general_service/src/pose_estimate/data_face/medi"+str(all_image_count)+".jpg",nimg)
        all_image_count+=1
        area=[]
        ...
        for idx in idx_list:
            ...
            # cv2.imwrite(f"{}")
            # cv2.imwrite(f"/home/zkw/general_service/src/pose_estimate/data_face/{person_index}.jpg",change_image)
            if start_work==False:
        ...
        if (not loc_quene.empty()):
            ...
            if start_work==False:
                if choose_face_or_pose_flag==1:
                    ...
                    loc_target,human_list,image_name=loc_quene.get()
                    goal=judge_arrive()
                    ...
                    if m<253:
                        rospy.loginfo("%s",loc_target)
                        rospy.loginfo("%s",image_name)
                        x=loc_target.target_pose.pose.orientation.x
                        y=loc_target.target_pose.pose.orientation.y
                        z=loc_target.target_pose.pose.orientation.z
                        w=loc_target.target_pose.pose.orientation.w
                        
                        [r,p,y]=ori_to_rpy(x,y,z,w)
                        rospy.loginfo("%f,%f.%f",r,p,y)
                        loc_target_pub.publish(loc_target)
                    else :
                        ok_flag=0
                        ok_goal=None
                        for index_tttt, i in enumerate(human_list):
                            goal=judge_arrive()
                            goal.x = i.target_pose.pose.position.x
                            goal.y=i.target_pose.pose.position.y 
                            judge_can_or_not_go_pub.publish(goal)
                            ok_goal=i
                            while True:
                                try:
                            ...
                            if m<253:
                                ok_flag=1
                                break
                            rospy.loginfo("the %d index  can not go is ",index_tttt)
                        if ok_flag==1:
                            loc_target_pub.publish(ok_goal)
                            x=ok_goal.target_pose.pose.orientation.x
                            y=ok_goal.target_pose.pose.orientation.y
                            z=ok_goal.target_pose.pose.orientation.z
                            w=ok_goal.target_pose.pose.orientation.w
                            
                            [r,p,y]=ori_to_rpy(x,y,z,w)
                            rospy.loginfo("%s",ok_goal)
                            rospy.loginfo("%s",image_name)
                            rospy.loginfo("%f,%f.%f",r,p,y)
                            
                            rospy.loginfo("i have published human_target")

                        else:
                            rospy.loginfo("sorry no place can go")
                        # loc_target_pub.publish(human_target)
...
def easy_convert(real_people_x,real_people_y):
    global machine_odom_now_y,machine_odom_now_x,mathince_theta
    tho_second2 = math.sqrt(real_people_x ** 2 + real_people_y ** 2)
    
    theta_second2=atan2(real_people_y, real_people_x)
    x_person,y_person=get_map_pose(machine_odom_now_x+tho_second2*cos(theta_second2-mathince_theta),machine_odom_now_y-tho_second2*sin(theta_second2-mathince_theta))    
    # y1=-y1
    ...
    goal1.target_pose.pose.position.x = x_person
    goal1.target_pose.pose.position.y = y_person
    xyzw = rpy2quaternion(0.0, 0.0, theta1)
    goal1.target_pose.pose.orientation.x = 0.0
    goal1.target_pose.pose.orientation.y = 0.0
    goal1.target_pose.pose.orientation.z = xyzw[2]
    goal1.target_pose.pose.orientation.w = xyzw[3]
    return goal1
def convert_machine_axis_to_world(results,width,height,x_mid,y_mid,depth,name):
    ...
    robot_states = get_map_pose_theta()
    theta1=0
    rpy = ori_to_rpy(0.0, 0.0, robot_states[2], robot_states[3])
    rospy.loginfo("this is no change_alpha %f",alpha)
    

    
    machine_x,machine_y=get_map_pose(machine_odom_now_x,machine_odom_now_y)
    actual_theta=0
    # d=depth[y_mid,x_mid]
    d_middle=depth[y_mid][x_mid]
    use_dmidle=0
    if d_middle>0.00001:
        average_depth=d_middle
        use_dmidle=1
    real_x,_,real_z=real_pose(x_mid,y_mid,average_depth)
    rospy.loginfo("real_x,real_z %f,%f",real_x,real_z)
    if use_dmidle==0:
        
        real_z+=0.15
    if 0<alpha<=0.25*pi or 1.75 *pi<alpha<=2*pi:
            # theta1=0
            alpha=0
            if use_dmidle==0:
            ...
            # if use_dmidle==0:
            real_z+=0.2
    actual_theta=alpha+rpy[2]    
    rospy.loginfo("this is  change_alpha %f",actual_theta)
    # z_second = real_person[2] - i* cos(alpha)
    # x_second = real_person[0] + i * sin(alpha)
    final_rslt=easy_convert(real_z,real_x)
    real_people_x=real_z
    real_people_y=real_x
    theta_second2=atan2(real_people_y, real_people_x)
    tho_second2 = math.sqrt(real_people_x ** 2 + real_people_y ** 2)
    ...
    map_x=x_person
    map_y=y_person
    reach_place2=[(map_x-1,map_y,0),(map_x,map_y+1,-math.pi/2),(map_x,map_y-1,math.pi/2),(map_x+1,map_y,math.pi),
                  (map_x-0.85,map_y,0),(map_x,map_y+0.85,-math.pi/2),(map_x,map_y-0.85,math.pi/2),(map_x+0.85,map_y,math.pi),
                  (map_x-0.95,map_y,0),(map_x,map_y+0.95,-math.pi/2),(map_x,map_y-0.95,math.pi/2),(map_x+0.95,map_y,math.pi),
                  (map_x-1.15,map_y,0),(map_x,map_y+1.15,-math.pi/2),(map_x,map_y-1.15,math.pi/2),(map_x+1.15,map_y,math.pi),
                  (map_x-1.25,map_y,0),(map_x,map_y+1.25,-math.pi/2),(map_x,map_y-1.25,math.pi/2),(map_x+1.25,map_y,math.pi),
                  (map_x-1.3,map_y,0),(map_x,map_y+1.3,-math.pi/2),(map_x,map_y-1.3,math.pi/2),(map_x+1.3,map_y,math.pi),
                  (map_x-1.32,map_y,0),(map_x,map_y+1.32,-math.pi/2),(map_x,map_y-1.32,math.pi/2),(map_x+1.32,map_y,math.pi),
                  (map_x-0.8,map_y,0),(map_x,map_y+0.8,-math.pi/2),(map_x,map_y-0.8,math.pi/2),(map_x+0.8,map_y,math.pi),]
    distance_list=[]
    theta_distance_list=[]
    for (x,y,theta) in reach_place2:
        distance_list.append((machine_x-x)**2+(machine_y-y)**2)
        distance_theta=int(abs(theta-actual_theta)/math.pi *180)
        theta_distance_list.append(min(distance_theta%360,360-distance_theta%360))
    rospy.loginfo("%s",theta_distance_list)

    for i in range(len(distance_list)):
        for j in range(i,len(distance_list),1):
            if theta_distance_list[i]>theta_distance_list[j]:
                reach_place2[i],reach_place2[j]=reach_place2[j],reach_place2[i] 
                theta_distance_list[i],theta_distance_list[j]=theta_distance_list[j],theta_distance_list[i]
                

    rospy.loginfo("the reach place2 lenght is %d",len(reach_place2))
    rospy.loginfo("last angle %f",reach_place2[0][2])
    rospy.loginfo("theta_list %s",theta_distance_list)
    goal_list=[]
    for i in reach_place2:   
        temp_x=i[0]
        temp_y=i[1]
        temp_z=i[2]
        ...
        goal1 = MoveBaseGoal()
        goal1.target_pose.header.frame_id = 'map'
        goal1.target_pose.pose.position.x = temp_x
        goal1.target_pose.pose.position.y = temp_y
        xyzw = rpy2quaternion(0.0, 0.0, temp_z)
        goal1.target_pose.pose.orientation.x = 0.0
        goal1.target_pose.pose.orientation.y = 0.0
        goal1.target_pose.pose.orientation.z = xyzw[2]
        goal1.target_pose.pose.orientation.w = xyzw[3]
        goal_list.append(goal1)
    return goal_list[0],goal_list
...
def mediapipe_task(image,mutex,name,width,height,mid_x,mid_y,depth):
    ...
    global mediapipe_detect_count,mediape_mutex,machine_odom_now_x,machine_odom_now_y,mathince_theta
    mutex.acquire()
    # cv2.imwrite("")
    # cv2.imwrite(f"/home/zkw/general_service/src/pose_estimate/data_face/55565600{person_index}.jpg",image)
    ...
    if results.pose_world_landmarks!=None:
        ...
        final_rslt,human_result=convert_machine_axis_to_world(results,width,height,mid_x,mid_y,depth,name)
        # rospy.loginfo("%s",final_rslt)
        # rospy.loginfo("%s",human_result)
        
    else :
            d=depth[mid_y][mid_x]
            d_list_zero=[]
            if d<eps:
                for i in range(-int(width/4)+1,int(width/4)-1,int(width/8)):
                    for j in range(-int(height/4)+1,int(height/4)-1,int(height/8)):
                        d_list_zero.append(depth[mid_y+j][mid_x+i])
            length_of_d_list=len(d_list_zero)
            for i in d_list_zero:
                if i<eps:
                    length_of_d_list-=1
            if length_of_d_list!=0:
                d=sum(d_list_zero)/length_of_d_list
            real_x,_,real_z=real_pose(mid_x,mid_y,d)
            final_rslt=easy_convert(real_z,real_x)
            real_people_x=real_z
            real_people_y=real_x
            tho_second2 = math.sqrt(real_people_x ** 2 + real_people_y ** 2)
            ...
            for i in reach_place2:   
                ...
                goal1.target_pose.pose.orientation.w = xyzw[3]
                goal_list.append(goal1)
            human_result=goal_list    
            # cv2.imwrite(f"/home/zkw/general_service/src/pose_estimate/data_face/medi00{person_index}.jpg",image)
            pass
    mutex.release()
    return final_rslt,human_result   
...
def mediapipe_detect(image:np.ndarray,width,height,mid_x,mid_y,depth,name):
    ...
    if (image.size==0):
        ...
        return
    try:
        cv2.imwrite(f"/home/zkw/general_service/src/pose_estimate/data_face/{name}{last_person}.jpg",image)
    except BaseException as e:
        pass
    image=cv2.imread(f"/home/zkw/general_service/src/pose_estimate/data_face/{name}{last_person}.jpg")
    mutex_new1=threading.Lock()
    mutex_new2=threading.Lock()
    if choose_face_or_pose_flag==1:
        task_me=Mediapipe_detect(target=mediapipe_task,image=image,mutex=mutex_new1,path1=f"/home/zkw/general_service/src/pose_estimate/data_face/{name}{last_person}.jpg",width=width,height=height,mid_x=mid_x,mid_y=mid_y,depth=depth)
        task_me.start()
        task_me.join()
        (pose_feature,human_feature)=task_me.return_results()
        tuple_all=(pose_feature,human_feature,f"{name}{last_person}")
        ...
        store_path=f"/home/zkw/general_service/src/pose_estimate/data_face/{name}{last_person}.jpg"
...
def mediapipe_detect_2(image:np.ndarray,width,height,mid_x,mid_y,depth,name):
    ...
    if (image.size==0):
        ...
        return
    try:
        cv2.imwrite(f"/home/zkw/general_service/src/pose_estimate/data_face2/{name}{last_person}.jpg",image)
    except BaseException as e:
        pass
    mutex_new1=threading.Lock()
    mutex_new2=threading.Lock()
    store_path=f"/home/zkw/general_service/src/pose_estimate/data_face2/{name}{last_person}.jpg"
    task_face=Face_detect(target=task3,mutex=mutex2,name=store_path)
    if choose_face_or_pose_flag==3:
        task_me=Mediapipe_detect(target=mediapipe_task,image=image,mutex=mutex1,path1=f"/home/zkw/general_service/src/pose_estimate/data_face2/{name}{last_person}.jpg",width=width,height=height,mid_x=mid_x,mid_y=mid_y,depth=depth)
        task_me.start()
        task_face.start()
        task_me.join()
        task_face.join()
...
def choose_face_or_pose_cb(msg:String):
    ...
    if choose_face_or_pose_flag==1:
        x1, y1 = get_map_pose(machine_odom_now_x, machine_odom_now_y)
        robot_states = get_map_pose_theta()
        rpy = ori_to_rpy(
            0.0, 0.0, robot_states[2], robot_states[3])
        theta1 =  rpy[2]
...
if __name__=="__main__":
    ...
    file_in_face_db=os.listdir('/home/zkw/general_service/src/pose_estimate/face_db')
    
    if len(file_in_face_db)!=0:
        for i in file_in_face_db:
            path_file=os.path.join('/home/zkw/general_service/src/pose_estimate/face_db',i)
            os.remove(path_file)
    # 用以删除文件
    ...
    now_pose_pub=rospy.Publisher("/general_service_now_position",MoveBaseGoal,queue_size=10)
