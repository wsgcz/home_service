...
def switch_on_cb(target):
    ...
    while(switch==1):
        ...
        if (( abs( x-odom_px ) < 0.05 and abs( y-odom_py ) < 0.05  and abs(mathince_theta-last_mathince_theta)<0.01) and switch ==1 ):
            ...
            reach_place1=[(machine_x+0.3*cos(angle), machine_y-0.3*sin(angle),math.pi/2),(machine_x+0.36*cos(angle), machine_y-0.36*sin(angle),math.pi/2),(machine_x+0.42*cos(angle), machine_y-0.42*sin(angle),math.pi/2),(machine_x+0.5*cos(angle), machine_y-0.5*sin(angle),math.pi/2)]
            for i in range(4):
...
def over_switch_cb(msg):
    global switch
    switch=2      
def choose_angle(msg:String):
    global mathince_theta,angle
    data=int(msg.data)
    angle=data*math.pi/180
    angle=mathince_theta+angle
    rospy.loginfo("this is angle %f",angle)
...
if __name__=="__main__":
    ...
    pub=rospy.Subscriber("general_service_laser",String, choose_angle,queue_size=10)
