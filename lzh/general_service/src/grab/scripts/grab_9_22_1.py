...
depth_intrin=None
depth_image=None
grab_corner={"kitchen":[11.945,-1.018],"bedroom":[4.694,3.733]}
grab_room=None
sayword_number=0
...
class YoloThread(threading.Thread):
    def __init__(self, target,mutex):
        self.func=target
        self.ans=None
        self.mutex=mutex
        super().__init__(target=target)
    def run(self) -> None:
        self.ans=self.func(self.mutex)
...
def first_put_down():
    ...
    control_arm_data.position[0]=0.52
    # rospy.loginfo("this is final_y %f",final_position_y)
    control_arm_data.position[1]=0
...
def calculate_postion_data(mutex):
    ...
    if len(yolo_result_message)!=0  and sayword_number==0:
...
def intel_realsense_image_and_depth():
    ...
    try:
        ...
        # 深度参数，像素坐标系转相机坐标系用到
            ...
            yolo_inferen_task=YoloThread(calculate_postion_data,yolo_mutex)
            yolo_inferen_task.start()
            draw_image=yolo_inferen_task.return_result()
            if cv2.waitKey(1)!=27:
                cv2.imshow("yolo_intermediate",draw_image)
