...
inline bool GoToWayPoint(ros::ServiceClient &&client_get_waypoint_name,
  ...
  if (client_get_waypoint_name.call(srv_name)) {
    ...
    if (!ac.waitForServer(ros::Duration(5.0))) {
      ...
      ac.sendGoal(goal);
      // ROS_INFO("45645665454565646544444444444444");
      double last_of_time=ros::Time::now().toSec();
      auto m =actionlib::SimpleClientGoalState(actionlib::SimpleClientGoalState::PENDING);
      // while (m==actionlib::SimpleClientGoalState::PENDING)
      // {
      // // ROS_INFO("45645665454565646544444444444444");

      // double now_of_time =ros::Time::now().toSec();
      // if ((now_of_time-last_of_time)>10)
      //   {
      //     // ac.cancelAllGoals();
      //     ROS_INFO("ok ,remake ");
      //     ac.stopTrackingGoal();
