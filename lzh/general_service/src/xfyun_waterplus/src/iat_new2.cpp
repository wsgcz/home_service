...
#include<cstring>
static bool bCue = false;	//true-语音识别前有提示音; false-语音识别前没有提示音
...
void on_result(const char *result, char is_last)
{
	if (result) 
    {
        ...
        if(index(str,"ood")){
			...
			sleep(1);
			ros::shutdown();
			// ROS_INFO("go to dining");
        }
        else if(index(str,"rink")){
            ros::param::set("name",2);
			sleep(1);
			ros::shutdown();
			// ROS_INFO("go to bedroom");
		}
        else if(index(str,"lean")){
            ros::param::set("name",3);
			sleep(1);
			ros::shutdown();   
	    }
    }
