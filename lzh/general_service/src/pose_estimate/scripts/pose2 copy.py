...
def image_callback(image_rgb,image_depth):
    ...
    global flag,person_index,people_can_get_count,loc_quene,all_work_count,person_num,face_db_store_name
    #all_work_count:第几帧图片
    global start_work
    if start_work!=True:
        return
    if all_work_count<=1:
        ...
        image = letterbox(image, 960, stride=64, auto=True)[0]
        # image_ = image.copy()
        # transforms.ToTensor()把图像转化为[0,1]的torch.float32类型
        image = transforms.ToTensor()(image)
        image = torch.tensor(np.array([image.numpy()]))
        image = image.to(device)
        # 将此存储转为 half 类型,Half是用16位表示浮点数的一种数据类型
        image = image.half()
        ...
        if (not loc_quene.empty()):
            ...
            if len(face_db_store_name)==0:               
                ...
                if now_face_name in face_db_store_name:
