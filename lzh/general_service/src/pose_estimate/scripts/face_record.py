...
def image_callback(image_rgb,image_depth):
    ...
    global flag,person_index,people_can_get_count,loc_quene,all_work_count,choose_face_or_pose_flag,loc_target_pub,face_feature_pub
    ...
    # 打开摄像头
        ...
        if (not loc_quene.empty()):
            ...
            if choose_face_or_pose_flag==1:
                ...
                face_feature,image_name=loc_quene.get()
                face_feature_pub.publish(face_feature)
                choose_face_or_pose_flag=0
                all_work_count=0
...
def mediapipe_detect(image:np.ndarray,width,height,mid_x,mid_y,depth,name):
    ...
    if choose_face_or_pose_flag==1:
        ...
        tuple_all=(pose_feature,f"{name}{last_person}.jpg")
        ...
        if face_feature!=None:
            tuple_all=(face_feature,f"{name}{last_person}.jpg")
            put_loc_thread=threading.Thread(target=put_loc_into_quene,args=(tuple_all,))
        # person_index+=1
            put_loc_thread.start()
    # if pose_feature!=None:
    #     tuple_all=(pose_feature,f"{name}{last_person}.img")
    #     # tuple_all=1
    #     put_loc_thread=threading.Thread(target=put_loc_into_quene,args=(tuple_all,))
    #     # person_index+=1
    #     put_loc_thread.start()
...
def choose_face_or_pose_cb(msg:String):
    global choose_face_or_pose_flag
    choose_face_or_pose_flag=msg.data
if __name__=="__main__":
    ...
    face_feature_pub=rospy.Publisher("/general_service_face_feature",String,queue_size=10)
